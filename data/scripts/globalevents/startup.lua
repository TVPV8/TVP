local globalevent = GlobalEvent("GameStartup")
		...
		repeat
			...
			if house then
				...
				if house:getOwnerGuid() ~= highestBidder then
					db.query("UPDATE `accounts` SET `failed_bid_count` = `failed_bid_count` + 1 WHERE `id` = " .. result.getNumber(resultId, "account_id"))
