...
void PrivateChatChannel::invitePlayer(const Player& player, uint32_t guid)
{
	invites.insert(guid);
}
...
bool Chat::talkToChannel(const Player& player, SpeakClasses type, const std::string& text, uint16_t channelId)
{
	...
	if ((type == TALKTYPE_CHANNEL_R2 || type == TALKTYPE_CHANNEL_R1) && !player.hasFlag(PlayerFlags::PlayerFlag_CanTalkRedChannel)) {
		type = TALKTYPE_CHANNEL_Y;
	}
